...
const economicMoatFactors = [
    "Brand Loyalty or Brand Recognition",
...
const getEconomicMoat = async (ticker) => {
  ...
  for (let queryTermIdx = 0; queryTermIdx < economicMoatFactors.length; queryTermIdx++) {
      const response = await vectorStore.similaritySearch(economicMoatFactors[queryTermIdx], 15, filter);
      console.log(economicMoatFactors[queryTermIdx]);
      console.log(filter);
      ...
      const QA_PROMPT = `You are a writing an investment analyst report on the ticker ${ticker}. Use the following pieces of context to write a section of the report on the topic of "${economicMoatFactors[queryTermIdx]}".
      ...
      reportParts.push(`## ${economicMoatFactors[queryTermIdx]}\n\n${chatResponse}\n\n${sourceInformation}`);
      conclusionParts.push(chatResponse);
  }

  // Conclusion
  const conclusionContext = conclusionParts.join(' ');
  const CONCLUSION_PROMPT = `Based on the analysis of all the economic moat factors, write a conclusion for the investment analyst report on the ticker ${ticker}.
  ...
  let fullReport = `# Economic Moat Investment Analyst Report on ${ticker}\n\n` + reportParts.join("\n\n");

  console.log(fullReport);

  // Create a PDF document
  const doc = new PDFDocument;
  doc.pipe(fs.createWriteStream(`${ticker}_Economic_Moat.pdf`));
