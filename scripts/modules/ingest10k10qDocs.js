...
const { extractSignificantWords } = require('../utils/ingest/keywordExtraction');
...
const get10KItemAndIngest = async (index, filings, ticker, company_id) => {
  ...
  for (let year in filings) {
    ...
    if (Array.isArray(filings[year]['10-Q'])) {
      const promises = filings[year]['10-Q'].map(async (link) => {    
        type = "10Q";
        const values = [company_id, type, year, new Date(), link.html, link.month];
        try {
          ...
          get10KItemTxtAndIngest(link.link, link.txt, item, document_id, ticker, type, year)
        );
  ...
  // We attempt to sever the connection here
}

const get10KItemTxtAndIngest = async (link, txt, item, document_id, ticker, type, year) => {
